...
async def backfill_discovery_preferences():
    ...
    2. Updates existing entries where brokerages or state is NULL with the user's data.
    """
        ...
        try:
            ...
            if users_without_prefs:
                ...
                for user in users_without_prefs:
                    # Initialize with the user's own brokerage and state
            ...
            # PHASE 2: Update existing entries where brokerages or state is NULL
            print("\n🔍 Phase 2: Checking for existing entries with NULL brokerages or state...")
            stmt_update = (
                select(DiscoveryPreferences, User.brokerage, User.state)
                .join(User, DiscoveryPreferences.user_id == User.id)
                .where(
                    or_(
                        DiscoveryPreferences.brokerages == None, 
                        DiscoveryPreferences.state == None
                    )
                )
            )
            
            result_update = await session.execute(stmt_update)
            prefs_to_update = result_update.all() # Returns list of tuples (DiscoveryPreferences, brokerage, state)
            
            if prefs_to_update:
                print(f"Found {len(prefs_to_update)} existing entries to update. Updating...")
                for prefs, user_brokerage, user_state in prefs_to_update:
                    if prefs.brokerages is None and user_brokerage:
                        prefs.brokerages = [user_brokerage]
                    if prefs.state is None and user_state:
                        prefs.state = user_state
                print(f"✅ Updated {len(prefs_to_update)} existing entries.")
            else:
                print("✅ No existing entries need backfilling.")
